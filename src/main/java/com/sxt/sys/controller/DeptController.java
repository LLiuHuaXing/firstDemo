...
//			map.put("value", 1);
//		}
		Integer maxOrderNum=this.deptService.getDeptMax();
	...
	public ResultObj addDept(@Valid DeptVo deptVo, BindingResult bindingResult) {
		//数据校证
	...
	public ResultObj updateDept(@Valid DeptVo deptVo,BindingResult bindingResult) {
		//数据校证
		if(bindingResult.hasErrors()){
			String errorInfo="";
			for (FieldError fieldError:bindingResult.getFieldErrors()){
	 ...
	 * @return
	 */
	@RequestMapping("updateDeptAvailable")
	public ResultObj updateDeptAvailable(DeptVo deptVo) {
		try {
			//System.out.println(deptVo.getId());
			deptService.updateDeptAvailable(deptVo);
			if(deptVo.getAvailable()==1){
