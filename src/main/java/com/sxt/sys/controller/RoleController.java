...
public class RoleController {
	...
	public ResultObj addRole(@Valid RoleVo roleVo, BindingResult bindingResult) {
		//数据校证
	...
	public ResultObj updateRole(@Valid RoleVo roleVo, BindingResult bindingResult) {
		//数据校证
		if(bindingResult.hasErrors()){
			String errorInfo="";
			for (FieldError fieldError:bindingResult.getFieldErrors()){
	...
	public DataGridView initPermissionByRoleId(Integer roleId) {
		...
		if(currentRolePermissions.size()>0) { //如果有ID就去查询 currentRolePermissions为空，则查询所有的数据
