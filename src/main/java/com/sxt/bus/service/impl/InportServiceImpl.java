...
public class InportServiceImpl extends ServiceImpl<InportMapper, Inport> implements InportService {
	...
	public boolean removeById(Serializable id) {
		...
		//根据销售商品id删除销售退货信息
		int OutputNumber=this.getBaseMapper().selectByInportid(id);
		//库存的算法  当前库存-进货单数量
		goods.setNumber(goods.getNumber()+OutputNumber-inport.getNumber());
		this.goodsMapper.updateById(goods);
		//根据进货id删除退货信息
		this.getBaseMapper().deleteOutputForInportid(id);
